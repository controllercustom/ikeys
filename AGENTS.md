...
## Web Flasher (`docs/`)
- **`docs/index.html`** — ESP Web Tools flasher page, served via GitHub Pages. Users connect their ESP32-S3 via WebSerial/WebUSB to flash firmware from a browser (Chrome/Edge only).
- **`docs/firmware/`** — Contains firmware binaries and manifests (`.json`), generated by the CI workflow `.github/workflows/deploy-pages.yml` on tag push (`v*`) or `workflow_dispatch`.
- **Manifests** (`docs/firmware/esp32s3.json`, `docs/firmware/m5stack_atoms3.json`) are NOT committed — they are built in CI from a Python script that reads the git tag as the version.
- **Flash layout per manifest**: bootloader at 0x0, partitions at 0x8000, boot_app0 at 0xE000, firmware at 0x10000.
- **Erase-first checkbox**: Toggles `btn.eraseFirst` on the `<esp-web-install-button>`. However, even when unchecked, flashing the `partitions.bin` at offset 0x8000 overwrites the partition table, which can cause ESP-IDF to re-initialize the NVS partition on next boot — **WiFi credentials and Preferences are lost** regardless of the checkbox state. This is a known limitation: writing the partition table always invalidates NVS. To preserve WiFi credentials across flashes, the partition table must not be written (i.e., omit `partitions.bin` from the manifest) or use OTA updates instead.
