...
## Overview
Assistive keyboard project (v1.0.4) using an ESP32-S3 that acts as a USB HID device (Keyboard/Mouse). It hosts a Web UI to trigger keystrokes on the connected computer.
...
## Web Flasher (`docs/`)
- **`docs/index.html`** — Flasher page using `esptool-js` (direct WebSerial, no `esp-web-tools` web component). Users connect via browser serial dialog and flash firmware over 921600 baud.
- **`docs/app.js`** — ES module that fetches `firmware.json` manifest, drives `ESPLoader`/`Transport` from `esptool-js`, flashes via `flashBegin`/`flashBlock`/`flashFinish` commands.
- **`docs/style.css`** — Dark theme styling for the flasher page.
- **`docs/firmware.json`** (not committed) — Single manifest generated by CI listing all available versions, flash layout, and board metadata. Fetched by `app.js` at runtime.
- Firmware binaries are stored in the **`gh-pages` branch** under `firmware/<VERSION>/<board>/` — this keeps binary blobs out of the source branch while preserving all historical versions across CI runs. The CI workflow pushes newly built firmware to `gh-pages` on every build.
- **Flash layout per manifest**: bootloader at 0x0, partitions at 0x8000, boot_app0 at 0xE000, firmware at 0x10000.
- **Erase-first checkbox**: Triggers `loader.eraseFlash()` before flashing. Even when unchecked, writing `partitions.bin` at offset 0x8000 overwrites the partition table, which can cause ESP-IDF to re-initialize the NVS partition on next boot — **WiFi credentials and Preferences may be lost** regardless of the checkbox state. To preserve WiFi credentials across flashes, use OTA updates instead.
